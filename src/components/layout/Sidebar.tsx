...
const userNav = [
  ...
  { path: '/dashboard/import', label: 'Import Accounts', icon: Plus, isNew: true },
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <button onClick={() => { setMobileOpen(!mobileOpen); setCollapsed(false); }}
      ...
      <aside className={`fixed top-0 left-0 z-50 h-screen bg-card border-r border-border flex flex-col transition-all duration-200 ${sidebarWidth} ${
        ...
        {isAdmin && (!collapsed || mobileOpen) && (
          <div className="mx-3 mt-3 px-2 py-1 rounded text-[10px] font-mono text-destructive bg-destructive/8 border border-destructive/15 text-center">Admin</div>
        )}
          ...
          {nav.map(item => {
            ...
            return (
              <Link key={item.path} to={item.path}
                ...
                } ${active ? 'bg-primary/8 text-primary border-l-primary' : 'text-muted-foreground border-l-transparent hover:bg-muted hover:text-foreground'}`}
                title={!showLabel ? item.label : undefined}>
                <item.icon className="w-4 h-4 flex-shrink-0" />
                {showLabel && <span className="flex-1">{item.label}</span>}
                {showLabel && itemAny.isNew && (
                  <span className="bg-primary/15 text-primary text-[9px] font-mono px-1.5 py-0.5 rounded">NEW</span>
                )}
                {showLabel && 'badge' in item && (item as any).badge > 0 && (
                  <span className="bg-primary/15 text-primary text-[10px] font-mono px-1.5 py-0.5 rounded-full">{(item as any).badge}</span>
                )}
              </Link>
            );
          })}
